...
export class STM32 {
    ...
    connect(port, baud, hex, options, callback) {
        ...
        if (self.options.no_reboot) {
            ...
            serial.connect(port, { bitrate: self.options.reboot_baud }, openInfo => {
                if (openInfo) {
    ...
    verify_chip_signature(signature) {
        ...
        if (this.available_flash_size > 0 && (this.hex.bytes_total >= this.available_flash_size)) {
            return true;
        } else {
            message = `binary larger (${this.hex.bytes_total}) than flash (${this.available_flash_size})`;
            return false;
        }
        console.log(message);
    ...
    verify_flash(first_array, second_array) {
        for (let i = 0; i < first_array.length; i++) {
            if (first_array[i] != second_array[i]) {
                console.log(
                    `Verification failed on byte: ${i} expected: 0x${first_array[i].toString(16)} received: 0x${second_array[i].toString(16)}`
                );
                return false;
            }
        }
    ...
    upload_procedure(step) {
        ...
        switch (step) {
            case 1:
                ...
                let send_counter = 0;
                let interval = setInterval(() => { // 200 ms interval (just in case mcu was already initialized), we need to break the 2 bytes command requirement
            ...
            case 2:
                ...
                self.send([self.command.get, 0xFF], 2, data => { // 0x00 ^ 0xFF
                    if (self.verify_response(self.status.ACK, data)) {
                        self.retrieve(
                            data[1] + 1 + 1,
                            data => { // data[1] = number of bytes that will follow [– 1 except current and ACKs]
                                console.log(
            ...
            case 3:
                ...
                self.send([self.command.get_ID, 0xFD], 2, data => { // 0x01 ^ 0xFF
                    if (self.verify_response(self.status.ACK, data)) {
                        self.retrieve(
                            data[1] + 1 + 1,
                            data => { // data[1] = number of bytes that will follow [– 1 (N = 1 for STM32), except for current byte and ACKs]
            ...
            case 4:
                    ...
                    if (self.options.erase_chip) {
                        ...
                        self.send([self.command.extended_erase, 0xBB], 1, reply => {
                            if (self.verify_response(self.status.ACK, reply)) {
                                ...
                                let max_address = self.hex.data[self.hex.data.length - 1].address + self.hex.data[self.hex.data.length - 1].bytes - 0x8000000, erase_pages_n = Math.ceil(max_address / self.page_size), buff = [], checksum = 0;
                                ...
                                buff.push(checksum);
                                console.log(
                                    `Erasing. pages: 0x00 - 0x${erase_pages_n.toString(16)}, checksum: 0x${checksum.toString(16)}`
                                );
                ...
                if (self.options.erase_chip) {
                    ...
                    self.send([self.command.erase, 0xBC], 1, reply => { // 0x43 ^ 0xFF
                        if (self.verify_response(self.status.ACK, reply)) {
                    ...
                    self.send([self.command.erase, 0xBC], 1, reply => { // 0x43 ^ 0xFF
                        if (self.verify_response(self.status.ACK, reply)) {
                            ...
                            let max_address = self.hex.data[self.hex.data.length - 1].address + self.hex.data[self.hex.data.length - 1].bytes - 0x8000000, erase_pages_n = Math.ceil(max_address / self.page_size), buff = [], checksum = erase_pages_n - 1;
            ...
            case 5:
                ...
                let blocks = self.hex.data.length - 1, flashing_block = 0, address = self.hex.data[flashing_block].address, bytes_flashed = 0, bytes_flashed_total = 0; // used for progress bar

                const write = function () {
                    if (bytes_flashed < self.hex.data[flashing_block].bytes) {
                        const bytes_to_write = ((bytes_flashed + 256) <= self.hex.data[flashing_block].bytes) ? 256 : (self.hex.data[flashing_block].bytes - bytes_flashed);

                        // console.log('STM32 - Writing to: 0x' + address.toString(16) + ', ' + bytes_to_write + ' bytes');

                        self.send([self.command.write_memory, 0xCE], 1, reply => { // 0x31 ^ 0xFF
                            if (self.verify_response(self.status.ACK, reply)) {
                                // address needs to be transmitted as 32 bit integer, we need to bit shift each byte out and then calculate address checksum
                                const address_arr = [(address >> 24), (address >> 16), (address >> 8), address];
                                const address_checksum = address_arr[0] ^ address_arr[1] ^ address_arr[2] ^ address_arr[3];

                                self.send(
                                    [address_arr[0], address_arr[1], address_arr[2], address_arr[3], address_checksum],
                                    1,
                                    reply => { // write start address + checksum
                                        if (self.verify_response(self.status.ACK, reply)) {
            ...
            case 6:
                ...
                let blocks = self.hex.data.length - 1, reading_block = 0, address = self.hex.data[reading_block].address, bytes_verified = 0, bytes_verified_total = 0; // used for progress bar
                ...
                const reading = function () {
                    if (bytes_verified < self.hex.data[reading_block].bytes) {
                        const bytes_to_read = ((bytes_verified + 256) <= self.hex.data[reading_block].bytes) ? 256 : (self.hex.data[reading_block].bytes - bytes_verified);

                        // console.log('STM32 - Reading from: 0x' + address.toString(16) + ', ' + bytes_to_read + ' bytes');

                        self.send([self.command.read_memory, 0xEE], 1, reply => { // 0x11 ^ 0xFF
                            if (self.verify_response(self.status.ACK, reply)) {
                                const address_arr = [(address >> 24), (address >> 16), (address >> 8), address];
                                const address_checksum = address_arr[0] ^ address_arr[1] ^ address_arr[2] ^ address_arr[3];
                                ...
                                self.send(
                                    [address_arr[0], address_arr[1], address_arr[2], address_arr[3], address_checksum],
                                    1,
                                    reply => { // read start address + checksum
                                        if (self.verify_response(self.status.ACK, reply)) {
                                            ...
                                            self.send(
                                                ...
                                                1,
                                                reply => { // bytes to be read + checksum XOR(complement of bytes_to_read_n)
                                                    if (self.verify_response(self.status.ACK, reply)) {
            ...
            case 7:
                ...
                self.send([self.command.go, 0xDE], 1, reply => { // 0x21 ^ 0xFF
                    if (self.verify_response(self.status.ACK, reply)) {
                        const gt_address = 0x8000000, address = [(gt_address >> 24), (gt_address >> 16), (gt_address >> 8), gt_address], address_checksum = address[0] ^ address[1] ^ address[2] ^ address[3];

                        self.send(
                            [address[0], address[1], address[2], address[3], address_checksum],
                            1,
                            reply => {
                                if (self.verify_response(self.status.ACK, reply)) {
                                    // disconnect
...
export class STM32DFU {
    ...
    controlTransfer(direction, request, value, _interface, length, data, callback, _timeout) {
        const self = this;
        ...
        switch (step) {
                                ...
                                if (data[4] == self.state.dfuDNBUSY) { // completely normal
                                    ...
                                    const waitForErase = setInterval(() => {
                                        ...
                                        self.controlTransfer(
                                            ...
                                            6,
                                            0,
                                            (data, error) => { // should stall/disconnect
            ...
            case 2:
                    ...
                    for (let j = 0; j < self.flash_layout.sectors[i].num_pages; j++) {
                        if (self.options.erase_chip) {
                            ...
                            for (let k = 0; k < self.hex.data.length; k++) {
                                const starts_in_page = self.hex.data[k].address >= page_start && self.hex.data[k].address <= page_end;
                                const end_address = self.hex.data[k].address + self.hex.data[k].bytes - 1;
                                const ends_in_page = end_address >= page_start && end_address <= page_end;
                                const spans_page = self.hex.data[k].address < page_start && end_address > page_end;
                ...
                const erase_page = function () {
                    ...
                    console.log(
                        `Erasing. sector ${erase_pages[page].sector}, page ${erase_pages[page].page} @ 0x${page_addr.toString(16)}`
                    );
            ...
            case 4:
                ...
                const write = function () {
                    if (bytes_flashed < self.hex.data[flashing_block].bytes) {
                        const bytes_to_write = ((bytes_flashed + 2048) <= self.hex.data[flashing_block].bytes) ? 2048 : (self.hex.data[flashing_block].bytes - bytes_flashed);
            ...
            case 5:
                ...
                const read = function () {
                    if (bytes_verified < self.hex.data[reading_block].bytes) {
                        const bytes_to_read = ((bytes_verified + 2048) <= self.hex.data[reading_block].bytes) ? 2048 : (self.hex.data[reading_block].bytes - bytes_verified);
